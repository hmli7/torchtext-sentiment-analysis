...
class SpeechModel(nn.Module):
    ...
    def __init__(self,n_phonemes,num_rnn, hidden_size, nlayers, embed_size = 40):
...
#         self.rnn = nn.LSTM(input_size = embed_size,hidden_size=hidden_size,num_layers=nlayers) # Recurrent network
    ...
    def forward(self, seq_batch):
        ...
        # pad sequence
        padded_sequence = pad_sequence(seq_batch)
        packed_sequence = pack_padded_sequence(padded_sequence,sequence_lens)

        h = packed_sequence
        for l in self.rnns:
            h, _ = l(h)
        output_padded, sequence_lens = pad_packed_sequence(h) # unpacked output (padded)
#         output_flatten = torch.cat([output_padded[:sequence_lens[i],i] for i in range(batch_size)]) # concatenated output
        scores_flatten = self.scoring(output_padded) # concatenated logits
        return scores_flatten, sequence_lens
...
def init_weights(net, weight_init,
                 ...
                 bias_init=partial(init.constant, val=0.0),
                 forget_bias_init=partial(init.constant, val=1.0)):
    """
    Args:
